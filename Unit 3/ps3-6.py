# -*- coding: utf-8 -*-
"""
Created on Fri Feb 21 21:09:28 2014
...
class robot:
    ...
    def move(self, motion, noise_flag = 0): # Do not change the name of this function

        # ADD CODE HERE
        # Check exception first
        if abs(motion[0]) > max_steering_angle :
            raise ValueError, 'Steering angle must be smaller than pi/4.0'
        if motion[1] < 0 :  # may not need, maybe car can back drive
            raise ValueError, 'Distance can not be negative'   
        # get necessary value
        result = robot(self.length) # not good, will not copy all of info of self
...
#        d = random.gauss(motion[1], self.distance_noise)
        if noise_flag == 0:
            alpha = motion[0]   # get steering angle
            d = motion[1]       # get distance
        else:   # add noise
            alpha = random.gauss(motion[0], self.steering_noise)
            d = random.gauss(motion[1], self.distance_noise)
        
        beta = (d/self.length)*tan(alpha)   # turning angle
    ...
    def sense(self, noise_flag = 0): #do not change the name of this function
        Z = []
        ...
        for i in range(len(landmarks)):
            ...
            if noise_flag == 0:
                ...
                bearing = atan2(dy, dx) - self.orientation + random.gauss(0, self.bearing_noise)
