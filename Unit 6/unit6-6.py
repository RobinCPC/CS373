...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        ### ENTER CODE HERE
        # extract estimate position of robot
        x = estimate[0]
        y = estimate[1]

        # get the vector of P1
        x1 = spath[index][0]
        y1 = spath[index][1]
        # get the vector of p2
        x2 = spath[index+1][0]
        y2 = spath[index+1][1]
        ...
        # in map, x positive direction is go down, and y position is go right
        plt.scatter(myrobot.y, -myrobot.x)
        plt.xlim([-1, 6])
        plt.ylim([-6, 1])
        plt.pause(1e-7)
