...
#        print "i = ", i
    ...
    for ind, step in enumerate(data):
        # sense first
        #print step[0]
        for m in step[0]:
            # update x
            Omega.value[2*ind][2*ind] += 1 / measurement_noise
            Omega.value[2*ind][2*N+2*m[0]] += -1 / measurement_noise
            Xi.value[2*ind][0] += -m[1] / measurement_noise

            Omega.value[2*N+2*m[0]][2*ind] += -1 / measurement_noise
            Omega.value[2*N+2*m[0]][2*N+2*m[0]] += 1 / measurement_noise
            Xi.value[2*N+2*m[0]][0] += m[1] / measurement_noise

            # updata y
            Omega.value[2*ind+1][2*ind+1] += 1 / measurement_noise
            Omega.value[2*ind+1][2*N+2*m[0]+1] += -1 / measurement_noise
            Xi.value[2*ind+1][0] += -m[2] / measurement_noise

            Omega.value[2*N+2*m[0]+1][2*ind+1] += -1 / measurement_noise
            Omega.value[2*N+2*m[0]+1][2*N+2*m[0]+1] += 1 / measurement_noise
            Xi.value[2*N+2*m[0]+1][0] += m[2] / measurement_noise
        # move next
        m = step[1]
        #update x
        Omega.value[2*ind][2*ind] += 1 / motion_noise
        Omega.value[2*ind][2*ind+2] += -1 / motion_noise
        Xi.value[2*ind][0] += -m[0] / motion_noise

        Omega.value[2*ind+2][2*ind] += -1 / motion_noise
        Omega.value[2*ind+2][2*ind+2] += 1 / motion_noise
        Xi.value[2*ind+2][0] += m[0] / motion_noise

        #update y
        Omega.value[2*ind+1][2*ind+1] += 1 / motion_noise
        Omega.value[2*ind+1][2*ind+2+1] += -1 / motion_noise
        Xi.value[2*ind+1][0] += -m[1] / motion_noise

        Omega.value[2*ind+2+1][2*ind+1] += -1 / motion_noise
        Omega.value[2*ind+2+1][2*ind+2+1] += 1 / motion_noise
        Xi.value[2*ind+2+1][0] += m[1] / motion_noise
