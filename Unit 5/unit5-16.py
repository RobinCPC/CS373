...
def twiddle(tol = 0.001): #Make this tolerance bigger if you are timing out!
    ...
    # Add code here
    # -------------
    params = [0, 0, 0]   # set of PID gain value
    dp = [1, 1, 1]  # set of gain tuing step

    # first run to get original error average
    best_err = run(params)
    #print best_err
    n = 0   # counting times
    ...
    while sum(dp) > tol:
        for i in xrange(len(params)):
            params[i] += dp[i]   # first set gain up
            err = run(params)
